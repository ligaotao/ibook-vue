...
export default {
  ...
  created() {
    ...
    // 这里导致列表计算出现问题
    // bus.$on(EVENT_TYPE.ITEM, this.onItemResized);

    // listen slot size change
    if (this.$slots.header || this.$slots.footer) {
      bus.$on(EVENT_TYPE.SLOT, this.onSlotResized);
    }
  },
  ...
  methods: {
    ...
    emitEvent(offset, clientSize, scrollSize, evt) {
      bus.$emit("scroll", evt, this.virtual.getRange());
